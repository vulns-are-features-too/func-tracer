...
var (
	...
	errTargetNotFound       = errors.New("couldn't find target")
	errMultipleTargetsFound = errors.New("multiple matching targets found")
...
func (r *runner) findTarget() (model.Symbol, error) {
	...
	if err != nil {
		return model.Symbol{}, fmt.Errorf("%w: %w", errTargetNotFound, err)
	}
...
func (r *runner) findTargetByName(fileURI string, name string) (model.Symbol, error) {
	...
	targets := r.idx.FindFunctionByName(fileURI, name)

	if len(targets) == 0 {
		return model.Symbol{}, fmt.Errorf(
